...
if __name__ == "__main__":
    ...
    # Adattato al nuovo output vettoriale numpy
    raw_samples = np.load('samples_combo.npy', allow_pickle=True)
    
    print("Spacchettamento dei bitstring in corso...")
    num_samples = len(raw_samples)
    model_samples = np.zeros((num_samples, N), dtype=np.float32)
    
    for i, s in enumerate(raw_samples):
        # L'operatore >> sposta i bit e & 1 estrae il valore esatto del singolo qubit
        for j in range(N):
            model_samples[i, j] = (s >> j) & 1

    model_spins = 1.0 - 2.0 * model_samples
    num_samples = model_spins.shape[0]
    ...
    model_wht_k1 = model_spins.mean(axis=0)
    ...
    for idxs in sampled_ops_k2:
        parity = model_spins[:, idxs[0]] * model_spins[:, idxs[1]]
        model_wht_k2.append(parity.mean())
    model_wht_k2 = np.array(model_wht_k2)
    ...
    for _ in range(num_k3_tests):
        ...
        parity_model = model_spins[:, idxs[0]] * model_spins[:, idxs[1]] * model_spins[:, idxs[2]]
        model_wht_k3.append(parity_model.mean())
    ...
    all_vals_seen = np.concatenate([target_plot_k1, model_wht_k1, target_plot_k2, model_wht_k2])
    lims_seen = [all_vals_seen.min() - 0.02, all_vals_seen.max() + 0.02]
    
    lims_k3_x = [min(target_wht_k3) - 0.02, max(target_wht_k3) + 0.02]
    lims_k3_y = [min(model_wht_k3) - 0.02, max(model_wht_k3) + 0.02]

    # --- CANVAS 1: 1-local (k=1) ---
    ax1.scatter(target_plot_k1, model_wht_k1, color='#1f77b4', alpha=0.6, edgecolors='none', label='1-local frequencies')
    ...
    ax1.set_ylabel(r"LinComb Samples $\langle \chi_1 \rangle_{LC}$")
    ax1.set_xlim(lims_seen)
    ax1.set_ylim(lims_seen)
    ax1.legend(loc="upper left")

    # --- CANVAS 2: 2-local (k=2) ---
    ax2.scatter(target_plot_k2, model_wht_k2, color='#ff7f0e', alpha=0.4, edgecolors='none', label='2-local parities')
    ...
    ax2.set_ylabel(r"LinComb Samples $\langle \chi_2 \rangle_{LC}$")
    ax2.set_xlim(lims_seen)
    ax2.set_ylim(lims_seen)
    ax2.legend(loc="upper left")

    # --- CANVAS 3: 3-local (k=3) 
    ax3.scatter(target_wht_k3, model_wht_k3, color='#2ca02c', alpha=0.5, edgecolors='none', label='3-local (Unseen)')
    ...
    ax3.set_ylabel(r"LinComb Predictions $\langle \chi_3 \rangle_{LC}$")
    ax3.set_xlim(lims_k3_x)
    ax3.set_ylim(lims_k3_y)
    ax3.legend(loc="upper left")

    # Aggiornato al nuovo modello e ultimo errore
    plt.suptitle("Spoofing IQP in MMD with LinCombApproximation\n 484 qubits D-WAVE dataset - MMD2 = 7.857e-04", y=0.98)
    plt.tight_layout()

    output_pdf = "spoof_tcdq_canvas_lincomb.pdf"
